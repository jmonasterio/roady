...
#
# Prereqs: Cloudflare credentials in ../cf-infra/.env
...
if [ -f ../cf-infra/.env ]; then
  set -a && . ../cf-infra/.env && set +a
fi
: "${CLOUDFLARE_API_TOKEN:?CLOUDFLARE_API_TOKEN not set — see DEPLOY.md (expected in ../cf-infra/.env)}"

# IPv4-first: Node 22 + undici prefers IPv6, which black-holes on this LAN
# and hangs wrangler's API calls. See ../cf-infra/AGENTS.md.
